...
const TagsPage = () => {
  ...
  const fetchClassificationChildrens = async (
  ...
  const fetchClassifications = async (setCurrent?: boolean) => {
    ...
    try {
      ...
      if (setCurrent && response.data.length) {
        ...
        fetchClassificationChildrens(response.data[0].name);
      }
    } catch (error) {
      const errMsg = getErrorText(
        error as AxiosError,
  ...
  useEffect(() => {
    ...
    fetchClassificationChildrens(ClassificationName);
  }, [ClassificationName]);
  ...
  const onClickClassifications = (category: Classification) => {
    ...
    fetchClassificationChildrens(category.name);
  ...
  useEffect(() => {
    fetchClassificationChildrens(currentCategoryName);
  }, [isAddingTag]);
